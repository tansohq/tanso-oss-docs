...
channel: last seven full days per unit against the seven before, month-to-date
...
the daily ceiling stays an alert. Spend that LiteLLM counts and spend Tanso
counts can differ slightly (LiteLLM prices from its own model map), so the
proxy may block a little before or after Tanso's alert.
