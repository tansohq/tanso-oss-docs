...
**Pulled.** Connect GitHub (a token with read access to pull requests; scope
is a comma-separated `owner/repo` list) or Linear (an API key; scope is
comma-separated team keys, or `*`). Sync pulls merged PRs / completed issues
for the window and re-pulls upsert; an hourly job covers the last three days.

**Posted.** Any CI job or script can record one:

```bash
curl -X POST https://YOUR-INSTANCE/api/v1/spend/outcomes \
  -H "Authorization: Bearer $JWT" -H 'Content-Type: application/json' \
  -d '{"kind":"CUSTOM","externalId":"deploy-2026-08-25-1","title":"Prod deploy","actorEmail":"alice@acme.com"}'
```

The same `externalId` again updates the outcome. Kinds: `PR_MERGED`,
`ISSUE_DONE`, `CUSTOM`.
...
**Report.** Per unit, spend (with descendants) over outcomes (with
descendants): cost per merged PR, per team, per window.
