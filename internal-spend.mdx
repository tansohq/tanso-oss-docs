...
<Frame caption="Internal spend → Usage: what your org sent each vendor, priced two ways">
...
**Internal spend → Connections → Connect vendor.** Paste an Anthropic admin key
...
**Internal spend → Usage** shows a window by model, by day, and by person, two ways:
...
**Internal spend → Reconcile** compares one period three ways per vendor: metered,
...
<Frame caption="Internal spend → Teams: allocation rolls up the tree; a person's Claude Code estimate stays beside it">
  <img src="/images/console/spend-teams.png" alt="Teams and budgets allocation table" />
</Frame>

**Internal spend → Teams.** A **unit** is a team, a project, or a person; units nest,
...
Budgets are checked after every sync and hourly; **Internal spend → Alerts** lists
...
Under **Internal spend → Teams → Settings**:
...
<Frame caption="Internal spend → Alerts: the weekly digest, previewed before it goes out">
...
how many alerts fired. **Internal spend → Alerts** previews it and has **Send now**.
...
1. **Internal spend → Connections → Connect vendor**, provider *LiteLLM*: the proxy URL
...
<Frame caption="Internal spend → Outcomes: merged PRs and done issues over allocated spend">
  <img src="/images/console/spend-outcomes.png" alt="Outcomes report: cost per outcome per unit, sources, recent outcomes" />
</Frame>

**Internal spend → Outcomes** puts shipped work next to what it cost. An outcome is a
...
**Internal spend → Usage → Savings.** Per model, the input side of the bill as it was billed —
...
<Frame caption="Internal spend → Usage → Savings: what caching saved, and the same tokens priced on another model">
...
<Frame caption="Internal spend → Feature P&L: build cost next to what the feature earns">
...
1. **Internal spend → Teams**, open (or create) a *project* unit and pick the feature
   it shipped under **Feature** — the same features monetization prices in
   plans.
2. **Internal spend → Feature P&L** (`GET /api/v1/spend/reports/pnl?from&to`): per project,
...
| Person level, worker notice, Slack webhook | `PUT /api/v1/spend/settings` | Console: Internal spend → Teams → Spend settings. |
