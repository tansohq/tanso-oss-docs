...
cd tanso-oss/deploy
cp .env.example .env     # set JWT_SECRET and APP_SECRETS_KEY, e.g. openssl rand -base64 48 each
...
[http://localhost:3000](http://localhost:3000), or the next free port if the
...
needed. Details in
[`ui/README.md`](https://github.com/tansohq/tanso-oss/blob/main/ui/README.md).
...
Your own customers are a different question. If you want their buying agents to
sign themselves up and pay without a human, that surface exists and is opt-in
per account: see [agent-serve](/agent-serve).
