...
Web Bot Auth, and A2A agent cards are roadmap. Reading back the individual
events behind a usage total is roadmap too: the events endpoint takes writes
only today. The `governance` block in pricing.json reports only what is true.
